...
struct
 ...
 fun isWellTyped t = 
    let
      ...
      (*        bool = 0    int = 1   fn = 2    generic = 3 (int/bool)-----varFunc = 4----Not Well typed = ~2          *)
      ...
      fun checkType (Z,l) = (1,~1,~1,l,Z)
          ...
          | checkType (P x,l) = let
                                ...
                                in
                                  if(typ = 1 orelse typ = 3 orelse typ = 4) then (1,~1,~1,L,Z) else (~2,~1,~1,L,Z)
                                end
          ...
          | checkType (IZ x,l) = let
                                  val (typ,i1,i2,L,p) = checkType (x,l)
                                in
                                  if(typ = 1 orelse typ = 3 orelse typ = 4) then (0,~1,~1,L,Z) else (~2,~1,~1,L,Z)
                                end
          ...
          | checkType (ITE (x1,x2,x3),l) = let
                                            ...
                                            in
                                              if(typ1 = 0 orelse typ1 = 3) then
                                                ...
                                                else if(typ2 <> 3 andalso typ3 <> 3 andalso typ2 <> 4 andalso typ3 <> 4) then (~2,~1,~1,L3,Z) 
                                                else if(typ2 = 3 orelse typ2 = 4) then
                                                  if(checkVar(x2) <> "") then 
          ...
          | checkType (APP(APP(x1,x2),x3),l) = let
                                              ...
                                              in if(typ1 = 4) then if(typ3 = 2) then (typ1,i4,3,L3,Z)
                                                    else (typ1,typ3,4,L3,Z)
                                              else if(typ2 <> ~2 andalso typ3 <> ~2) then
          ...
          | checkType (APP (VAR x1,x2),l) = let
                                                val (typ1,i11,i12,L1,p1) = checkType (x2,l)
                                                val (typ2,i21,i22,L2,p2) = if(find(L1,x1) = ~1) then
                                                                              if(typ1 <> 2) then
                                                                                (4,typ1,3,insert(L1,x1,2),Z)
                                                                              else (4,i12,3,insert(L1,x1,2),Z)
                                                                           else if(find(L1,x1) = 3) then
                                                                            (4,typ1,3,L1,Z)
                                                                          else (~2,~1,~1,L1,Z)
                                            in
                                              if(x1 = checkVar(x2)) then (~2,~1,~1,L2,Z)
                                              else (typ2,i21,i22,L2,p2)
                                            end
  ...
  (*    (n,i1,i2,l,t)   *)
    ...
    | fromString s = 
      let
        ...
        val t = parser(tokenised,[])
        val truthVal = isWellTyped(t)
      in
        t
      end
